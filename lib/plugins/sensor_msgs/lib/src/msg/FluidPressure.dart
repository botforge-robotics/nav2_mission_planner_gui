// Generated by ros_to_dart_converter
// Do not edit manually

import 'dart:convert';
import 'package:ros2_msg_utils/ros2_msg_utils.dart';

import 'package:std_msgs/msg.dart' as std_msgs;

class FluidPressure extends RosMessage<FluidPressure> {
  std_msgs.Header header;
  double fluid_pressure;
  double variance;

  static FluidPressure $prototype = FluidPressure();

  // Constructor
  FluidPressure(
      {std_msgs.Header? header, double? fluid_pressure, double? variance})
      : header = header ?? std_msgs.Header(),
        fluid_pressure = fluid_pressure ?? 0.0,
        variance = variance ?? 0.0;

  @override
  String get fullType => 'sensor_msgs/msg/FluidPressure';

  @override
  String get messageDefinition => '''std_msgs/Header header
float64 fluid_pressure
float64 variance''';

  @override
  int getMessageSize() {
    return (header.getMessageSize() + 8 + 8).toInt();
  }

  @override
  Map<String, dynamic> toJson() => {
        'header': header.toJson(),
        'fluid_pressure': fluid_pressure,
        'variance': variance
      };

  @override
  String toJsonString() => json.encode(toJson());

  @override
  FluidPressure fromJson(Map<String, dynamic> jsonMap) {
    return FluidPressure(
        header: std_msgs.Header()
            .fromJson(jsonMap['header'] as Map<String, dynamic>),
        fluid_pressure: jsonMap['fluid_pressure'] as double,
        variance: jsonMap['variance'] as double);
  }
}
