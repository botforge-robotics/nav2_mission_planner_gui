...
class ServiceClient<
  ...
  Future<Response> call(Request request) async {
    ...
    if (checkExists && name != '/rosapi/services') {
...
Future<bool> serviceExists(Ros2 ros2, String serviceName) async {
  ...
  try {
    ...
    final needle = serviceName.startsWith('/')
        ? serviceName
        : '/$serviceName';
    final bare = needle.substring(1);
    return response.services.any((s) =>
        s == serviceName || s == needle || s == bare || s.endsWith(bare));
  } catch (e) {
    // If rosapi enumeration fails (common on web), do not block the real call.
    return true;
  } finally {
