...
class LaunchManager extends ChangeNotifier {
  ...
  Future<bool> startMapping(BuildContext context) async {
    ...
    // Ensure NavProMini launch ref is used even if prefs are stale
    var launchRef = settings.mappingLaunchFile.trim();
    if (!launchRef.contains('/')) {
      launchRef = DefaultSettings.defaultMappingLaunchFile;
    }

    final parts = launchRef.split('/');
    if (parts.length != 2) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('Invalid launch file format: $launchRef'),
    ...
    final args = settings.mappingArgs.isEmpty
        ? DefaultSettings.defaultMappingArgs
        : settings.mappingArgs
            .where((a) => a['name'] != 'use_rviz')
            .toList();
    ...
    try {
      ...
      if (response.success) {
        ...
        _activeLaunches[response.unique_id] =
            '${parts[0]}/${parts[1]} ${args.map((arg) => '${arg['name']}:=${arg['value']}').join(' ')}';
        notifyListeners();
  ...
  Future<bool> startNavigation(BuildContext context, String mapName) async {
    ...
    var launchRef = settings.navigationLaunchFile.trim();
    if (!launchRef.contains('/')) {
      launchRef = DefaultSettings.defaultNavigationLaunchFile;
    }
    ...
    if (parts.length != 2) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('Invalid navigation launch file format: $launchRef'),
          backgroundColor: Colors.red.withOpacity(0.9),
        ),
      );
      return false;
    ...
    try {
      ...
      final baseArgs = settings.navigationArgs.isEmpty
          ? DefaultSettings.defaultNavigationArgs
      ...
      final args = [
        ...baseArgs,
        {'name': 'map', 'value': mapName.endsWith('.yaml') ? mapName : '$mapName.yaml'},
      ];
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('Failed to start navigation: ${response.message}'),
          backgroundColor: Colors.red.withOpacity(0.9),
        ),
      );
      return false;
